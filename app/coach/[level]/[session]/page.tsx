...
function cleanDrillName(drill: string): string {
  return drill.replace(/ \| Fun Youth Basketball Drills from the Jr\. NBA available in the MOJO App/gi, "");
}
...
export default function SessionPage({ params }: { params: { level: string; session: string } }) {
  ...
  const nextSession = sessionId < 12 ? getSession(params.level, sessionId + 1) : null;
